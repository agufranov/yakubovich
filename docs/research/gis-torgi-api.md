...
2. Нельзя просто «пройти всё постранично». Нужно **нарезать пространство запросов**
   на срезы меньше 10 000 записей — по комбинации `biddType` × окно дат
   (`pubFrom`/`pubTo`) × при необходимости регион (`dynSubjRF`; одноименное
   `subjectRFCode` из ответа как параметр не работает).
...
**Фильтры: проверено поштучно 2026-08-25** (метод мусорного значения; скрипты
`tools/probe/verify-*.mts`):

| Параметр | Статус | Заметки |
|---|---|---|
| `biddType` | ✅ работает | мусорный код -> 0 |
| `lotStatus` | ✅ работает | `PUBLISHED` -> 2195 |
| `pubFrom` / `pubTo` | ✅ работает | **строго `yyyy-MM-dd`**; ISO/epoch/ru-формат -> 400. Окно в 3 дня ~ 700 лотов 229-ФЗ |
| `dynSubjRF` | ⚠️ работает с оговоркой | валидный код фильтрует (`77` -> 5072), но **мусорное значение молча игнорируется** (-> 10000). Опечатка в коде региона не даст ошибку — даст полную выдачу |
| `catCode` | ✅ работает | мусор -> 0 |
| `subjectRFCode` | ❌ игнорируется | несмотря на то, что так называется поле в ответе |
| `noticeDateFrom` | ❌ игнорируется | |
| `biddEndFrom` | ⚠️ существует | 400 на все опробованные форматы даты — формат не найден |

Срезы комбинируются: `biddType=229FZ&pubFrom=2026-08-01&dynSubjRF=77` -> 88.

**Рецепт полной исторической загрузки**: идти окнами `pubFrom`/`pubTo` назад по
времени (окно подбирать так, чтобы срез был < 10 000; для 229-ФЗ хватает недели);
если окно все равно упирается в потолок — дробить его по `dynSubjRF`. Архив источника
глубокий: `pubTo=2026-01-01` тоже дает потолок.
